...
const CardDatas = ({ key, product }) => {
  ...
  return (
    <>
      <div
        className="w-full bg-white overflow-hidden relative group cursor-pointer"
        onMouseEnter={() => setChangePhoto(true)}
        onMouseLeave={() => setChangePhoto(false)}
        key={key}
      >
        <div className="relative w-full aspect-[3/4]">
          ...
          <Link to={`/details/${product._id}`}>
            <button className="flex items-center gap-3 absolute left-1/2 bottom-4 transform -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-all duration-500 ease-in-out bg-white text-black py-3 px-3 text-sm font-thin whitespace-nowrap">
              <IoBagAddOutline /> ADD TO BAG
            </button>
          </Link>
        </div>
        <Link to={`/details/${product._id}`}>
          <div className="p-4 flex flex-col gap-2">
            ...
            <div className="flex items-center justify-between">
              <p className="text-xs text-red-500">{product?.category?.name}</p>
              <p className="font-light text-gray-300 text-xs line-through">
                ₹{product?.price}
              </p>
            </div>
          </div>
